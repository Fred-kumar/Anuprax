...
// ── MongoDB Connection ─────────────────────────────────────
mongoose.connect(process.env.MONGODB_URI || "mongodb://localhost:27017/echoline")
...
async function sendOTP(email, code) {
  await transporter.sendMail({
    ...
    to: email,
    subject: "EchoLine — Verification Code",
    html: `
...
app.post("/api/send-otp", async (req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: "Failed to send OTP. Check EMAIL_USER and EMAIL_PASS env vars." });
  }
});
